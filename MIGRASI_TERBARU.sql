...
-- ============================================================================
-- BAGIAN 6: FIX - PUBLIC FORM CAN READ CUTI & BRANCH DATA (SECURITY DEFINER)
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_cuti_preview()
RETURNS TABLE (
  id UUID, employee_id UUID, jenis TEXT,
  tanggal_mulai DATE, tanggal_selesai DATE,
  status TEXT, tanggal_list JSONB,
  nama TEXT, branch_id UUID
)
LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE
AS $$
  SELECT c.id, c.employee_id, c.jenis, c.tanggal_mulai,
         c.tanggal_selesai, c.status, c.tanggal_list,
         e.nama, e.branch_id
  FROM public.cuti c
...
RETURNS TABLE (
  nama TEXT, kuota_cuti_hari_kerja INTEGER, kuota_cuti_akhir_pekan INTEGER
)
LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE
AS $$

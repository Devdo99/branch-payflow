...
export const getJenisCuti = (value?: string | null) =>
  JENIS_CUTI.find((j) => j.value === value) || {
    ...JENIS_CUTI[JENIS_CUTI.length - 1],
    label: value || "Lainnya",
  };
...
export const getStatusCuti = (value?: string | null) =>
  STATUS_CUTI.find((s) => s.value === value) || {
    value: value || "diajukan",
    label: value || "Diajukan",
...
export const STATUS_ABSEN: {
  ...
  // Dibuat otomatis dari jadwal cuti yang disetujui — tetap bisa diedit manual
  {
    value: "cuti",
    label: "Cuti",
    short: "C",

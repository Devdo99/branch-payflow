          ...
          <Dialog open={isOpen} onOpenChange={(open) => (!open ? handleClose() : setIsOpen(true))}>
            ...
            <DialogContent className="rounded-2xl">
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  saveMutation.mutate({ nama_jabatan: namaJabatan, tunjangan_jabatan: Number(tunjangan) });
                }}
                className="space-y-4 pt-2"
              >
                <div className="space-y-1.5">
                  <Label htmlFor="nama_jabatan" className="text-xs font-semibold uppercase tracking-wider text-slate-500">Nama Jabatan</Label>
                  <Input
                    id="nama_jabatan"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="tunjangan_jabatan" className="text-xs font-semibold uppercase tracking-wider text-slate-500">Nominal Tunjangan (Rp)</Label>
                  <Input
                ...
                >
                  {saveMutation.isPending ? <Loader2 className="animate-spin h-5 w-5 mx-auto" /> : "Simpan Jabatan"}
                </Button>
              </form>
            </DialogContent>
          </Dialog>
          ...
          <Table className="min-w-[500px]">
            <TableHeader>
              <TableRow className="bg-slate-50/80 dark:bg-slate-900/60 hover:bg-transparent">
                <TableHead className="font-semibold text-slate-900 dark:text-slate-100">Nama Jabatan</TableHead>
                <TableHead className="font-semibold text-slate-900 dark:text-slate-100">Tunjangan Jabatan</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                listJabatan.map((j: any) => (
                  <TableRow key={j.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors">
                    <TableCell className="font-semibold text-slate-900 dark:text-slate-100">{j.nama_jabatan}</TableCell>

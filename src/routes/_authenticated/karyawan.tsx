      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Kelola karyawan aktif dan nonaktif, serta atur cabang dan jabatan.
            </p>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm text-slate-500">
            <span>
              Menampilkan{" "}
              <span className="font-semibold text-slate-900">{filteredEmployees.length}</span> dari{" "}
              <span className="font-semibold text-slate-900">{employees.length}</span> karyawan
            </span>
            {branchFilter !== "all" && (
              <span className="rounded-full border border-slate-200 bg-slate-100 px-2 py-1 text-xs text-slate-700">
                Cabang: {branches.find((b: any) => b.id === branchFilter)?.nama}
              </span>
            )}
          </div>
        </div>
          ...
          <TableBody>
            {isLoading ? (
              ...
              filteredEmployees.map((item: any) => {
                ...
                const jabatanName =
                  listJabatan.find((j: any) => j.id === item.jabatan_id)?.nama_jabatan || "-";
                return (
                  <TableRow key={item.id} className="group transition-colors hover:bg-slate-50">
                    ...
                    <TableCell>
                      ...
                      <div className="mt-2 grid gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 text-xs text-slate-600 md:hidden">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className={item.aktif ? "text-emerald-700" : "text-rose-600"}>
                    ...
                    <TableCell className="hidden md:table-cell">
                      <Badge>{jabatanName}</Badge>
                    </TableCell>
                    <TableCell className="hidden md:table-cell">
                      {getMasaKerja(item.tanggal_masuk)}
                    </TableCell>
                    <TableCell className="hidden md:table-cell">

...
function AppProsesGajiPage() {
  ...
  const { data: listJabatan = [] } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('jabatan' as any).select('*').order('nama_jabatan')
      if (error) throw error
      return (data as any[]) || []
    }
  })
  ...
  useEffect(() => {
    if (dbEmployees) {
      setEmployees(dbEmployees.map(emp => {
        ...
        const jab = (listJabatan as any[]).find((j: any) => j.id === empJabatanKey || j.nama_jabatan === empJabatanKey)
        ...
        allowanceTypes.forEach((alw: any) => {
          ...
          const isEligible = !alw.catatan || alw.catatan === 'GLOBAL' || (jabatanName && alw.catatan.split(',').map((j: string) => j.toLowerCase().trim()).includes(jabatanName.toLowerCase().trim()))
        ...
        deductionTypes.forEach((ded: any) => {
          ...
          const isEligible = !ded.catatan || ded.catatan === 'GLOBAL' || (jabatanName && ded.catatan.split(',').map((j: string) => j.toLowerCase().trim()).includes(jabatanName.toLowerCase().trim()))
          if (!isEligible) return
        ...
        return { ...emp, jabatan: jabatanName, jabatan_tunjangan: jabatanTunjangan, component_inputs, custom_allowances, grandTotal: gajiBersih }
      }))
    }
      ...
      <Dialog open={isDetailOpen} onOpenChange={(open) => !open ? closeDetail() : setIsDetailOpen(true)}>
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Ringkasan payroll untuk karyawan ini.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-2">
              <div className="text-sm font-medium text-slate-700">{detailEmp?.jabatan || 'Tidak ada posisi'}</div>
              <div className="text-xs text-slate-500">Gaji Pokok: {formatIDR(detailEmp?.gaji_pokok || 0)}</div>
              <div className="text-xs text-slate-500">Tunjangan Jabatan: {formatIDR(detailEmp?.jabatan_tunjangan || 0)}</div>
              <div className="text-xs text-slate-500">Total Tunjangan: {formatIDR(detailBreakdown?.totalTunjangan || 0)}</div>
              <div className="text-xs text-slate-500">Total Potongan: {formatIDR(detailBreakdown?.totalPotongan || 0)}</div>
              <div className="text-xs font-semibold text-slate-900">Total Bersih: {formatIDR(detailBreakdown?.gajiBersih || 0)}</div>
            </div>
            <div className="space-y-2">
              <div className="text-xs font-semibold text-slate-700">Masukan nilai manual/per-faktor</div>
              {allowanceTypes.filter(alw => checkIsEligible(alw.catatan, detailEmp?.jabatan ?? '')).map(alw => (
                <div key={alw.id} className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-2 py-2">
                  <div className="text-xs text-slate-600">{alw.nama}</div>
                  {alw.metode === 'fixed' ? (
                    <div className="text-xs font-medium text-slate-700">{formatIDR(alw.nominal_default)}</div>
                  ) : (
                    <Input
                      type="number"
                      ...
                      onChange={(e) => detailEmp && handleInputChange(detailEmp.id, alw.id, e.target.value)}
                    />
                  )}
                </div>
              ))}
          ...
          <div className="flex justify-end gap-2 pt-2 border-t mt-2">
            ...
            <Button onClick={closeDetail} className="bg-slate-900 text-white hover:bg-slate-800">Tutup</Button>
          </div>
        </DialogContent>
      </Dialog>
          ...
          <Table className="min-w-max">
            <TableHeader>
              <TableRow className="hover:bg-transparent bg-slate-50/50">
                ...
                <TableHead key={alw.id} className="hidden md:table-cell text-center min-w-[120px] border-t-2 border-t-emerald-400 bg-emerald-50/30">
                ...
                {deductionTypes.map(ded => (
                  <TableHead key={ded.id} className="hidden md:table-cell text-center min-w-[120px] border-t-2 border-t-rose-400 bg-rose-50/30">
              ...
              {isLoading ? (
                ...
                <>
                  <TableRow key={emp.id} className="group hover:bg-slate-50/50 transition-colors">
                    <TableCell className="sticky left-0 bg-white group-hover:bg-slate-50/95 z-10 space-y-2 border-r border-slate-100 transition-colors">
                      <div>
                        ...
                        <div className="text-[11px] text-slate-500 leading-tight mt-0.5">
                          ...
                          {emp.jabatan_tunjangan > 0 && (
                            <div className="text-[11px] text-teal-700 font-medium mt-1">Tunjangan: {formatIDR(emp.jabatan_tunjangan)}</div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <Button variant="secondary" size="sm" className="h-6 px-2 text-[10px] shadow-none border-slate-200 text-slate-700 hover:bg-slate-100 md:hidden" onClick={() => openDetail(emp)}>
                          Detail
                        </Button>
                      </div>
                    </TableCell>
                    ...
                    {allowanceTypes.map(alw => {
                      ...
                      return (
                        <TableCell key={alw.id} className="hidden md:table-cell text-center align-top pt-4">
                          {!isEligible ? (
                    ...
                    {deductionTypes.map(ded => {
                      ...
                      return (
                        <TableCell key={ded.id} className="hidden md:table-cell text-center align-top pt-4">
                          {ded.metode === 'fixed' ? (
                  ...
                  <TableRow className="md:hidden bg-slate-50/60">
                    <TableCell colSpan={allowanceTypes.length + deductionTypes.length + 5} className="py-3 px-3 text-xs text-slate-600">
                      <div className="grid gap-2">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <span className="font-medium">Total Tunjangan</span>
                          <span className="font-semibold">{formatIDR(getPayrollBreakdown(emp).totalTunjangan)}</span>
                        </div>
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <span className="font-medium">Total Potongan</span>
                          <span className="font-semibold">{formatIDR(getPayrollBreakdown(emp).totalPotongan)}</span>
                        </div>
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <span className="font-medium">Bersih</span>
                          <span className="font-semibold text-slate-900">{formatIDR(emp.grandTotal)}</span>
                        </div>
                        <div className="flex gap-2 flex-wrap">
                          <Button variant="outline" size="sm" className="h-7 px-2 text-[10px] shadow-none border-teal-200 text-teal-700 bg-teal-50/30 hover:bg-teal-100" onClick={() => handleAddCustomAllowance(emp.id)}>
                            <Plus className="w-3 h-3 mr-1" /> Penyesuaian
                          </Button>
                          <Button variant="secondary" size="sm" className="h-7 px-2 text-[10px] shadow-none border-slate-200 text-slate-700 hover:bg-slate-100" onClick={() => openDetail(emp)}>
                            Detail
                          </Button>
                        </div>
                      </div>

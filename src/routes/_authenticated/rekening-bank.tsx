...
function getStatusBadge(status: BankStatus) {
  switch (status) {
    ...
    case "perlu_dicek_ulang":
      return (
        <Badge className="bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/20 shadow-sm rounded-lg py-1 px-2.5">
          Perlu Cek Ulang
        </Badge>
      );
  }
                    ...
                    <TableCell>
                      {item.nomor_rekening ? (
                        <div className="flex flex-col">
                          <span className="font-mono text-sm font-semibold text-slate-950 dark:text-slate-100">
                            {item.nomor_rekening}
                          </span>

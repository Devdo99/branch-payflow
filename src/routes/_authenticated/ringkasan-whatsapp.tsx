    ...
    <div className="flex flex-col gap-6 p-6">
          ...
          <TableBody>
            {isLoading ? (
              ...
              runs?.map((run) => {
                ...
                const progressPercent = totalItems === 0 ? 0 : Math.round((sentItems / totalItems) * 100)
                ...
                return (
                  <TableRow key={run.id}>
                    ...
                    <TableCell className="text-center">
                      <Badge variant={isAllSent ? "default" : "secondary"} className={isAllSent ? "bg-green-500" : ""}>
                    ...
                    <TableCell className="text-right">
                      <Button variant="outline" size="sm" className="gap-2" onClick={() => openDetail(run)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Detail Pengiriman - {selectedRun ? formatPeriodeDisplay(selectedRun.periode) : ''}</DialogTitle>
          </DialogHeader>
                ...
                {selectedRun?.payroll_items?.length === 0 ? (
                  ...
                  selectedRun?.payroll_items?.map((item: any) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>{item.employees?.whatsapp || <span className="text-muted-foreground italic">Kosong</span>}</TableCell>
                      <TableCell className="text-center">
                        {item.slip_dibuat ? (
